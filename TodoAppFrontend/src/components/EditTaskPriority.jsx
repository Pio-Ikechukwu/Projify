  ...
  const handleUpdate = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:3000/projects/${projectId}/priorities/${priority.id}`,
        {
          method: "PUT",
          headers: {
      ...
      onPriorityUpdated(data.priority);
      closePriorityEditModal();
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-black/40 flex justify-center items-center z-[100]">
      <div className="border bg-white border-gray-300 rounded-sm min-h-[300px] p-10 w-1/2">
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={closePriorityEditModal}>
        ...
        <div className="flex gap-4">
          ...
          <button
            onClick={closePriorityEditModal}
